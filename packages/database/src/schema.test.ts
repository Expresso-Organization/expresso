...
describe.skipIf(!mongoUrl)("MongoDB schema", () => {
  ...
  it("reruns without replacing existing seed changes", async () => {
    ...
    expect(result.existing).toEqual(["0001_initial_collections", "0002_generation_ledger_amount_constraint", "0003_analytics_rate_and_notification_preferences", "0004_job_import_metadata", "0005_job_source_ats_providers", "0007_job_source_boards"]);
