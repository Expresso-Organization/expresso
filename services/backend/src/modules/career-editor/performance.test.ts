...
describe.skipIf(!enabled)("career editor performance budgets", () => {
  ...
  beforeAll(async () => {
    ...
    viewId = (await viewService.create(userId, viewCategory.id, 1, { name: "기본", type: "table", filter: null, sorts: [{ propertyId: scoreId, direction: "asc", nulls: "last" }], groupPropertyId: null, groupOrder: [], recordOrder: [], visiblePropertyIds: [titleId, scoreId], propertyOrder: [titleId, scoreId], columnWidths: {}, gallery: null, board: null, timeline: null })).id;
