...
import { careerCategoryApiDefinitions } from "./properties.js";

export function mapMongoCategory(category: CareerCategoryDoc, recordCount = 0) {
  return CareerCategorySchema.parse({ id: category._id, key: category.key, name: category.name, icon: category.icon, defaultView: category.defaultView, isSystem: category.isSystem, propertySchema: category.propertySchema, propertySchemaV2: careerCategoryApiDefinitions(category), schemaVersion: category.schemaVersion, sortOrder: category.sortOrder, version: category.version, recordCount });
}
