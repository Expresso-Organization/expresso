    ...
    const recipeId = await inTransaction(this.context, async (tx) => {
      ...
      for (const [sectionOrder, section] of planned.draft.sections.entries()) {
        ...
        for (const [itemOrder, item] of section.items.entries()) {
          ...
          const itemId = randomUUID(); items.push({ _id: itemId, userId, recipeSectionId: sectionId, orderNo: itemOrder, pointText: item.pointText, content: item.kind ? { kind: item.kind, ...(item.metric ? { metric: item.metric } : {}), ...(item.link ? { link: item.link } : {}), ...(item.caption ? { caption: item.caption } : {}) } as unknown as JsonObject : null, evidence: cited.map(({ source }) => ({ sourceType: source.type, sourceId: source.id })), locked: false, editedBy: "ai", updatedAt: now });
