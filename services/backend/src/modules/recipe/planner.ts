...
const SYSTEM = [
  ...
  "- role: 이 섹션이 지면에서 맡는 역할. hero · profile · research · experience · projects · contact · other 중 하나.",
  "- presentation: 보여주는 형식. 역할에 맞는 것 중 하나를 고르고, 확신이 없으면 비운다.",
  "  hero: hero-big-statement · hero-split · hero-metric · hero-image · hero-profile-card",
  "  projects: project-par · project-case-study · project-artifact · project-metric · project-process-timeline · project-comparison",
  "  experience: career-timeline · career-by-org · career-role · career-outcome · career-project-linked",
  "  research: project-case-study · project-par · metric-group · metric-annotated · metric-single · body",
  "  profile: profile · hero-profile-card · body · quote / contact: contact-action · footer · profile",
  "  other: body · image-gallery · quote · metric-single · metric-before-after · metric-group · metric-bars · metric-gauge · metric-annotated · skill-tags · skill-categories · skill-evidence · skill-project-linked · skill-stack-table",
  "- interactionOpportunity: 내용을 더 잘 이해시키는 상호작용. 필요 없으면 null.",
  "",
  "## 항목의 종류",
  "항목마다 kind 를 적을 수 있다. 없으면 point(글)다.",
  "- metric: 재료에 **그대로 있는** 수치. metric { label, value, unit?, note? } 를 함께 적는다. 값을 만들어 내지 않는다.",
  "- link: 재료에 있는 주소(논문 · 데모 · 저장소). link { label, url } 를 함께 적는다.",
  "- media: 스크린샷 · 사진이 있으면 살아나는 자리. 자산은 사용자가 놓는다 — caption 에 무엇이 들어갈 자리인지 적는다.",
  "",
  "## 지어내지 않는다",
  ...
  async plan(context: PlannerContext): Promise<RecipePlanResult> {
    ...
    const draft = RecipeDraftSchema.parse({
      sections: titles.map((title, index) => ({
        ...
        role: index === 0 ? "hero" : index === 1 ? "projects" : "other",
