...
import { PortfolioPlanSchema, RecipeEditResultSchema, RecipeEditSchema, RecipeSchema, type Recipe, type RecipeEdit } from "@expresso/contracts";
...
export class MongoRecipeService implements RecipeApi {
  ...
  async generate(userId: string, brewId: string, idempotencyKey: string) {
    ...
    const recipeId = await inTransaction(this.context, async (tx) => {
      ...
      const plannedSections: Array<{ id: string; title: string; purpose: string; takeaway: string; evidenceIds: string[]; contentPattern: typeof planned.draft.sections[number]["contentPattern"]; interactionOpportunity: string | null; targetLength: number }> = [];
      for (const [sectionOrder, section] of planned.draft.sections.entries()) {
        ...
        plannedSections.push({ id: sectionId, title: section.title, purpose: section.purpose, takeaway: section.takeaway, evidenceIds: [...evidenceIds], contentPattern: section.contentPattern, interactionOpportunity: section.interactionOpportunity, targetLength: section.targetLength });
      }
      const sourceAt = (number: number) => plannerContext.sources[number - 1];
      const portfolioPlan = PortfolioPlanSchema.parse({ version: 1, target: { company: plannerContext.freeTitle ?? plannerContext.company?.name ?? "지원 회사", role: plannerContext.freeTitle ? "자유 포트폴리오" : plannerContext.jobTitle ?? "지원 직무", primaryReaders: plannerContext.freeTitle ? ["일반 방문자"] : ["채용 담당자", "실무 리드"], decisionGoal: plannerContext.freeBrief ? plannerContext.freeBrief.slice(0, 500) : "실제 근거를 바탕으로 인터뷰 대상으로 판단하게 한다" }, positioning: planned.draft.plan.positioning, requirementCoverage: planned.draft.plan.requirementCoverage.flatMap((entry) => { const requirement = sourceAt(entry.requirementSource); return requirement?.type === "requirement" ? [{ requirementId: requirement.id, requirementLabel: requirement.label, priority: entry.priority, evidenceIds: entry.evidenceSources.flatMap((number) => { const source = sourceAt(number); return source && (source.type === "record" || source.type === "answer") ? [source.id] : []; }), coverage: entry.coverage, reason: entry.reason }] : []; }), narrative: { arc: planned.draft.plan.narrativeArc, sectionOrder: plannedSections.map(({ id }) => id) }, sections: plannedSections, claims: planned.draft.plan.claims.map((claim, index) => ({ id: `claim-${index + 1}`, intent: claim.intent, evidenceIds: claim.evidenceSources.flatMap((number) => { const source = sourceAt(number); return source && (source.type === "record" || source.type === "answer") ? [source.id] : []; }), allowedNumbers: claim.allowedNumbers })), exclusions: planned.draft.plan.exclusions, unusedEvidence: planned.draft.unused.flatMap(({ source, reason }) => sourceAt(source)?.type === "record" ? [{ evidenceId: sourceAt(source)!.id, reason }] : []), companyContext: plannerContext.companyResearch, rationale: planned.draft.plan.rationale });
      ...
      recipe.portfolioPlan = portfolioPlan as JsonObject; recipe.planningManifest = manifest as JsonObject;
