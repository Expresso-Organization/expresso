// NOTE(agent-chat-framework): 원본 구조와 스타일을 유지하고 UI 문구를 한국어로 바꾸고 미지원 작업은 capabilities로 숨깁니다.
"use client";
...
const ThreadScrollToBottom: FC = () => {
  return (
    <ThreadPrimitive.ScrollToBottom render={<TooltipIconButton tooltip="최근 메시지로" variant="outline" className="aui-thread-scroll-to-bottom dark:border-border dark:bg-background dark:hover:bg-accent absolute -top-12 z-10 self-center rounded-full p-4 disabled:invisible" />}><ArrowDownIcon /></ThreadPrimitive.ScrollToBottom>
  );
};
  ...
  return (
    <div className="aui-composer-action-wrapper relative flex items-center justify-between">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <AuiIf condition={(s) => s.thread.isRunning}>
          <ComposerPrimitive.Cancel render={<Button type="button" variant="default" size="icon" className="aui-composer-cancel size-7 rounded-full" aria-label="응답 중지" />}><SquareIcon className="aui-composer-cancel-icon size-3.5 fill-current" /></ComposerPrimitive.Cancel>
        </AuiIf>
      </div>
    </div>
  );
...
const AssistantActionBar: FC = () => {
  return (
    ...
    >
      ...
      <AuiIf condition={s => s.thread.capabilities.reload}><ActionBarPrimitive.Reload render={<TooltipIconButton tooltip="다시 생성" />}><RefreshCwIcon /></ActionBarPrimitive.Reload></AuiIf>
...
const UserActionBar: FC = () => {
  return (
    ...
    >
      <AuiIf condition={s => s.thread.capabilities.edit}><ActionBarPrimitive.Edit render={<TooltipIconButton tooltip="Edit" className="aui-user-action-edit" />}><PencilIcon /></ActionBarPrimitive.Edit></AuiIf>
    </ActionBarPrimitive.Root>
  );
};
