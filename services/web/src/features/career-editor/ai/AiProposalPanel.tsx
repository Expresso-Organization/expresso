...
export function AiProposalPanel({ recordId, documentVersion, selectedBlockIds = [], announcedProposal, client = defaultClient, onApplied }: {
  recordId: string;
  ...
  function rememberFocus() { returnFocus.current = document.activeElement instanceof HTMLElement ? document.activeElement : null; }
  ...
  async function create(nextPrompt = prompt) {
    if (!nextPrompt.trim()) return;
    setIssue(null); setStatus("streaming"); rememberFocusIfMissing();
    try { const next = await client.create(recordId, { prompt: nextPrompt.trim(), selection: { blockIds: [...selectedBlockIds] } }); setProposal(next); setStatus(next.status ?? "ready"); selectAll(next); }
  ...
  return <section className={styles.panel} aria-label="Expresso AI 편집 제안">
    ...
    {proposal && ["ready", "conflicted", "applied"].includes(status) ? <div className={styles.review}><div><strong>{proposal.summary}</strong><small>본문 {proposal.commands.length}건 · 속성 {proposal.propertyChanges.length}건</small></div><AiProposalDiff proposal={proposal} commandIndexes={commandIndexes} propertyChangeIndexes={propertyIndexes} onCommandToggle={(index) => setCommandIndexes((current) => { const next = new Set(current); next.has(index) ? next.delete(index) : next.add(index); return next; })} onPropertyToggle={(index) => setPropertyIndexes((current) => { const next = new Set(current); next.has(index) ? next.delete(index) : next.add(index); return next; })} /><div className={styles.reviewActions}>{status === "applied" ? <button type="button" onClick={() => void undo()}>{undoConfirm ? "되돌리기 확인" : "변경 되돌리기"}</button> : <><button type="button" onClick={() => void reject()}>거절</button><button type="button" disabled={commandIndexes.size + propertyIndexes.size === 0} onPointerDown={rememberFocus} onClick={() => void apply()}>선택한 변경 적용</button></>}</div></div> : null}
