...
function definition(type: CareerPropertyDefinitionV2["type"], config: Record<string, unknown> = {}): CareerPropertyDefinitionV2 { return { id, key: "field", name: "테스트 속성", type, required: false, system: false, config, order: 0, version: 1, deletedAt: null }; }
function setup(type: CareerPropertyDefinitionV2["type"], value: CareerPropertyValueV2 | null = null, config: Record<string, unknown> = {}) { const onCommit = vi.fn(async () => undefined); render(<PropertyValueEditor definition={definition(type, config)} value={value} onCommit={onCommit} />); return onCommit; }
...
describe("PropertyValueEditor", () => {
  ...
  it.each(["formula", "rollup", "created_time", "updated_time"] as const)("renders %s as read-only", (type) => { const computed = type === "formula" || type === "rollup"; setup(type, computed ? { type, value: 3, diagnostics: [] } : { type, value: "2026-09-01T00:00:00.000Z" }); expect(screen.getByRole("status", { hidden: true }).textContent).toBe(computed ? "3" : "2026-09-01T00:00:00.000Z"); });
});
