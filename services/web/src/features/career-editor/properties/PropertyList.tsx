...
export function PropertyList({
  ...
  return <section className={styles.propertyList} aria-label="문서 속성">
    ...
    <div className={styles.propertyRow}><label><Icon name="circle-half" size={13}/>상태</label><span className={styles.metaValue}>{current.status === "draft" ? "초안" : current.status === "organized" ? "정리됨" : "검증됨"}</span></div>
    {visible.map((definition) => <div className={styles.propertyRow} key={definition.id}><label><Icon name={propertyIcon(definition.type)} size={13}/>{definition.name}</label>{definition.type==="relation"?<RelationEditor recordId={current.id} propertyId={definition.id} definition={definition.config as unknown as CareerRelationDefinition} value={asV2Value(definition,current.properties[definition.key])?.type==="relation"?(asV2Value(definition,current.properties[definition.key]) as Extract<CareerPropertyValueV2,{type:"relation"}>).value:[]} onConflict={()=>window.location.reload()} onCommit={async targetIds=>{const response=await fetch(`/api/career/records/${current.id}/relations`,{method:"PUT",headers:{"content-type":"application/json","if-match":`"v${current.version}"`},body:JSON.stringify({propertyId:definition.id,targetIds})});if(!response.ok)throw new Error(`${response.status} 관계를 저장하지 못했습니다.`);const payload=await response.json() as {data:CareerRecordListItem};setCurrent(previous=>({...previous,...payload.data}));}}/>:<PropertyValueEditor definition={definition} value={asV2Value(definition, current.properties[definition.key])} onCommit={async (value) => { const properties = { ...current.properties, [definition.key]: value }; if (value === null) delete properties[definition.key]; await savePatch({ properties }); }} />}</div>)}
    <div className={styles.propertyRow}><label><Icon name="chat-circle-dots" size={13}/>출처</label><span className={styles.metaValue}>{current.origin === "manual" ? "직접 작성" : current.origin === "ai" ? "AI 정리" : current.origin === "interview" ? "AI 대화" : "가져오기"}</span></div>
    <div className={styles.propertyRow}><label><Icon name="link-simple" size={13}/>사용처</label><span className={styles.metaValue}>{current.usedInCount > 0 ? `${current.usedInCount}곳` : "—"}</span></div>
