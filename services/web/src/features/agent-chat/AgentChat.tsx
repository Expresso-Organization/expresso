  ...
  return <ChatContext.Provider value={state}><AssistantRuntimeProvider runtime={runtime}>
    <section className={`acf-scope ${styles.root} ${standalone ? styles.standalone : styles.panel}`} aria-label="에이전트 채팅">
      <div className={styles.main}>
        ...
        {access?.enabled && access.consentRequired ? <div role="alert" className={styles.apiKeyField}><p>채팅을 시작하려면 AI 사용 동의가 필요합니다.</p><small>대화와 연결된 커리어 기록을 AI 모델에 전달합니다. 설정의 AI 사용 동의에서 ‘커리어 기록 사용’을 켜 주세요.</small><Link href="/account">설정에서 동의 확인</Link></div> : null}
