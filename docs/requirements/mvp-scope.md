...
Project Create API에서 사용 기술과 주요 성과는 각각 `string[]`으로 표현한다.
두 필드는 생략하거나 빈 배열로 입력할 수 있고, 각각 최대 20개까지 허용한다.
각 항목은 trim 후 빈 값과 같은 배열 안의 동일 문자열 중복을 허용하지 않으며,
입력 순서와 응답 순서를 보장하지 않는다.

학력·이력 주요 활동 / 성과에 대해서는 다음 사항을 이 문서에서 정하지 않는다.
...
- 프로젝트 사용 기술: `string[]`, 최대 20개, trim 결과 중복 금지, 순서 미보장
- 프로젝트 주요 성과: `string[]`, 최대 20개, trim 결과 중복 금지, 순서 미보장
- 학력·이력 주요 활동 / 성과: 복수값

학력·이력 주요 활동 / 성과의 최대 또는 최소 항목 수, 중복 허용 여부, 순서의 의미
및 구체적인 API 표현 방식은 해당 API 설계 전에 결정한다.
