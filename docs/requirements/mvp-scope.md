...
각 Category 관련 링크의 단일 / 복수 여부는
현재 MVP 핵심 ERD를 막는 항목으로 보지 않고 후속 결정 사항으로 둔다.
...
남아 있는 각 Category 관련 링크의 단일 / 복수 여부는
현재 MVP 핵심 ERD를 바로 막는 항목으로 취급하지 않는다.

관련 링크를 실제 MVP 구현 및 DB와 API 설계 범위에 포함하는 경우
해당 설계 전에 별도로 결정한다.
