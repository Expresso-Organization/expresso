...
프로젝트 링크, 대표 이미지 및 Section 콘텐츠는 현재 확정된 의미까지만 표현한다.
이들의 실제 저장 구조를 미리 정규화하거나 물리 구조로 확정하지 않는다.

## 17. 현재 미결정 사항

다음 사항은 현재 논리 ERD의 문서화를 막지 않으며 미결정 상태를 유지한다.

- 프로젝트 링크의 단일 또는 복수 여부
- 반복값의 중복 허용 여부
- 반복값 순서의 의미
- 반복값의 최대 개수
- 반복값의 별도 순서 속성
