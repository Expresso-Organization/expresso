...
현재 1차 MVP의 논리 모델에는 다음 11개 엔티티를 포함한다.
...
| 비동기 생성 작업 영속화 결정 | 사용자와 생성 결과 포트폴리오를 참조하는 포트폴리오 생성 작업 엔티티 |

포트폴리오 생성 작업은 핵심 비즈니스 개념을 표현하는 개념 ERD에는 추가하지
않는다. 비동기 처리 상태를 DB에 영속하는 독립 애플리케이션 Resource이므로 논리
ERD부터 포함한다.
...
Project 사용 기술과 주요 성과의 입력 정책은
`docs/requirements/api/project-api.md`에서 정의한다. 해당 정책은 API와 서비스
계층에서 검증하므로 논리 엔티티에 UNIQUE, 순서 또는 최대 개수 속성을 추가하지
않는다.

학력·이력 주요 활동·성과에 대해서는 다음 정책이 미결정 상태를 유지한다.
...
포트폴리오 생성 작업은 비동기 Portfolio 생성 요청의 처리 상태와 생성 결과를
추적하는 논리 엔티티이다.
...
| 생성 작업 식별자 | PK | 필수 |
| 사용자 식별자 | FK → 사용자 | 필수 |
| 상태 | 일반 속성 | 필수, `PENDING`, `PROCESSING`, `SUCCEEDED`, `FAILED` 중 하나 |
| 생성된 포트폴리오 식별자 | FK → 포트폴리오 | 선택 |
| 생성 시점 | 일반 속성 | 필수 |
| 완료 시점 | 일반 속성 | 선택 |

사용자 한 명은 포트폴리오 생성 작업을 0개 이상 가질 수 있으며, 각 생성 작업은
정확히 한 사용자에게 귀속된다. 각 생성 작업은 생성 결과 포트폴리오를 0개 또는
1개 참조한다. 하나의 포트폴리오를 참조할 수 있는 생성 작업 수에는 현재 고유
제약을 두지 않는다.

생성 요청을 접수하면 생성 작업을 먼저 생성한다. 유효한 생성 결과를 얻은 경우
포트폴리오, 포트폴리오 섹션 및 포트폴리오 생성 재료를 저장하고 생성 작업을
`SUCCEEDED` 상태와 생성된 포트폴리오에 연결한다. 생성에 실패한 경우 정상 완료된
포트폴리오를 저장하지 않고 생성 작업을 `FAILED` 상태로 유지한다.

상세 상태 전이, 재시도, Queue 및 Worker 구조는 현재 논리 모델에서 결정하지 않는다.
...
| 사용자 — 포트폴리오 생성 작업 | 1 : 0..N | 비식별 관계 | 각 생성 작업은 정확히 한 사용자에게 귀속된다. |
| 포트폴리오 — 포트폴리오 생성 작업 | 1 : 0..N | 비식별 관계 | 각 생성 작업은 생성 결과 포트폴리오를 0개 또는 1개 참조한다. |
...
사용자 (1)
  ...
  └─ 비식별 ── (0..N) 포트폴리오
                            ...
                            ├─ (1..N) 포트폴리오 생성 재료
                            │                  └─ (각 항목이 커리어 기록 1개 참조)
                            └─ (0..N) 포트폴리오 생성 작업의 생성 결과

커리어 기록 (1) ── 식별 ── (0..N) 포트폴리오 생성 재료
포트폴리오   (1) ── 식별 ── (1..N) 포트폴리오 생성 재료
사용자       (1) ── 비식별 ── (0..N) 포트폴리오 생성 작업
포트폴리오   (1) ── 비식별 ── (0..N) 포트폴리오 생성 작업
각 포트폴리오 생성 작업은 생성 결과 포트폴리오를 0..1개 참조한다.
```

![Expresso 1차 MVP 논리 ERD](./images/expresso-1st-mvp-logical-erd.png)

> 본 이미지는 임시 파일이며, 포트폴리오 생성 작업은 추후 이미지에 반영할 예정이다.
...
- 포트폴리오 생성 작업의 상태는 `PENDING`, `PROCESSING`, `SUCCEEDED`, `FAILED` 중 하나이다.
- 정상 완료된 생성 작업은 생성된 포트폴리오를 참조한다.
- 실패한 생성 작업은 정상 완료된 포트폴리오를 생성하지 않는다.
...
- 비동기 생성 상태는 포트폴리오 결과물과 분리된 포트폴리오 생성 작업에서 관리한다.
...
- 학력·이력 주요 활동·성과 반복값의 최대 개수
- 학력·이력 주요 활동·성과 반복값의 별도 순서 속성
- 실제 데이터베이스 타입
- 포트폴리오 섹션 콘텐츠 저장 형식
- 대표 이미지 저장 형식
- FK 삭제 정책
...
- 학력·이력 주요 활동·성과 반복값의 중복·순서 정책 반영 방식
- 대표 이미지의 저장 및 수명주기 구조
- 포트폴리오 섹션 콘텐츠의 저장 형식
- 포트폴리오 생성 재료의 복합 PK와 참조 무결성 구현 방식
- 포트폴리오 생성 작업의 식별자, 상태, 시점 및 결과 참조 구조
- 포트폴리오 생성 작업의 FK와 조회 인덱스
- FK 삭제 및 Cascade 정책
- 필요한 인덱스와 고유 제약
