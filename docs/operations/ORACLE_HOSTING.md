...
- `infra/compose.server.yaml` — 전용 MongoDB · Redis. 포트는 루프백에만 열고
  비밀번호는 `.env`에서 받는다.
...
ssh Oracle-Server 'cd ~/expresso && docker compose --env-file infra/.env -f infra/compose.server.yaml up -d --wait mongodb redis'
