...
CSP는 한 가지가 다르다. 디자인 문서는 컴파일러가 심는 고정 스크립트 하나를
실행한다. 견본을 화면에 들어올 때 재생하고 나가면 멈추는 일, 수치를 자리마다
굴러 올라오는 릴로 바꾸는 일을 한다.

```text
default-src 'none'; style-src 'unsafe-inline'; script-src 'sha256-…'
```

`'unsafe-inline'`이 아니라 **그 스크립트 내용의 sha256만** 허용한다. 스크립트는
계약에서 온 문자열을 한 글자도 끼워 넣지 않는 상수라 해시가 판마다 같고, 주입된
스크립트는 해시가 달라 그대로 차단된다. 스크립트가 실행되지 않으면 아무 클래스도
붙지 않아 문서는 정지 상태로 온전히 읽힌다.

자유 HTML과 생성된 포트폴리오 페이지에는 이 예외를 적용하지 않는다. 그쪽은
사용자와 모델이 쓴 내용을 담으므로 `script-src`를 열지 않는다.
