...
`/portfolio/new` 는 화면 없이 브루를 만들고 곧바로 디자인 단계로 보낸다. 제목은
임시값으로 시작하고 설명은 비운다 — 무엇을 겨냥하는지는 생성 뒤 편집기에서 받는다.
기존 `/brew/new`는 호환 진입점으로 남는다.
...
- 추천
- Expresso 기본 — 기본 디자인 3종과 Design Prompts 기반 스타일 30종
- 유명 웹사이트
- 내 디자인
- 회사 웹사이트
...
각 디자인 카드에는 이름, 출처 유형, 대표 특징, 추천 이유와 그 디자인의 지면 ·
글자 · 강조 색으로 이름을 조판한 포스터 썸네일을 표시한다. 참고 디자인은 출처 회사
마크를 지면 오른쪽 가장자리에 반쯤 걸쳐 크게 깔고, 이름은 그 위에 얹는다 —
카탈로그 항목은 마크 이름만 주고 그림은 화면이 가진다. 아는 회사가 아니면
마크 없이 그린다. 목록 응답에는 문서를
싣지 않는다 — `DESIGN.html` 전문은 인스펙터에서 고른 판만 따로 불러온다.
...
표지는 번호를 붙이지 않고 디자인 이름, 시각 방향, 그리고 서체 · 지면 · 행동 ·
형태를 실제 크기와 실제 색으로 그린 견본 넷을 담는다. 그 뒤 열 절에 번호를 붙인다.
...
「요소와 표시 방식」 절은 완성 페이지 한 장이 아니라 **요소 갤러리**다. 7.9의 기본
요소와 표시 방식을 요소별로 묶어 표시 방식마다 견본 하나를 그린다. 사용자는
자기 포트폴리오에 넣을 Hero, 프로젝트, 수치, 경력, 기술, 본문·인용·연락이 이
디자인에서 어떻게 보이는지 고르기 전에 확인한다.

문안은 모든 디자인에서 같은 값을 쓴다. 콘텐츠를 고정해 차이가 디자인에서만
오게 한다.

HTML 탭은 `미리보기`와 `코드 보기`를 제공한다. 실행은 자유 HTML과 같은 소독과
sandbox 경계를 사용한다.

CSP는 자유 HTML보다 넓다. 디자인 문서는 스스로 보여야 할 것이 있어 네 자리를
연다.
...
| 자리 | 여는 이유 |
| --- | --- |
| `script-src` | 견본을 화면에 들어올 때 재생하고 나가면 멈추는 일, 수치를 릴로 바꾸는 일 |
| `font-src`·`style-src` | 선언한 서체를 실제로 불러온다. 불러오지 않으면 타이포그래피 견본은 그 서체가 설치된 기계에서만 사실이다 |
| `img-src` | 이미지 자리에 넣는 고정 샘플 사진 |

`script-src`는 `'unsafe-inline'`이 아니라 **스크립트 내용의 sha256만** 허용한다.
스크립트는 계약에서 온 문자열을 한 글자도 끼워 넣지 않는 상수라 해시가 판마다
같고, 주입된 스크립트는 해시가 달라 그대로 차단된다. 실행되지 않으면 아무 클래스도
붙지 않아 문서는 정지 상태로 온전히 읽힌다.

나머지 셋은 지정한 호스트만 연다. 사진과 서체가 뜨지 않아도 문서 구조와 값은
그대로 읽힌다. 그 대신 문서는 두 외부 호스트에 의존하게 되므로, 사진 출처는 출처
절에 남긴다.

자유 HTML과 생성된 포트폴리오 페이지에는 이 예외를 적용하지 않는다. 그쪽은
사용자와 모델이 쓴 내용을 담으므로 `default-src 'none'`을 그대로 둔다.
...
> **2026-09-01 정정.** 이 절은 레시피가 요소 종류와 표시 방식까지 정하는 것으로
> 그려져 있다. 그 결정은 뒤집혔다 — 레시피는 **어떤 내용이 어떤 순서로**
> 들어갈지만 정하고, 지면의 모양은 01에서 고른 디자인 안에서 03 생성이 정한다.
> 아래 7.6 · 7.8 · 7.9 의 표시 방식 어휘와 폭 · 강조는 구현하지 않는다.
> 근거는 §20 의 2026-09-01 줄에 있다.
>
> **2026-09-15 되돌림 — 종류와 형식은 다시 02 가 정한다.** 내용과 순서만으로는
> 수치 · 그림 · 링크가 있는 지면(참고: 개인 포트폴리오 사이트)이 나오지 않았다.
> 문장에 **종류**(점 · 수치 · 미디어 · 링크)를, 섹션에 **역할**(히어로 · 프로필 ·
> 연구 · 경력 · 프로젝트 · 연락 · 그 밖)과 **보여주는 형식**(7.9 의 어휘 그대로)을
> 두고 사용자가 고른다. 화면은 세 영역이다 — 왼쪽에서 내용(기록 · 미디어 · 공고
> 요건)을 고르고, 가운데에서 문장과 종류를 고치고, 오른쪽에서 섹션의 형식을
> 파란 박스 와이어프레임으로 고른다. 어느 디자인을 골랐든 형식 목록은 같다 —
> 7.8 의 디자인별 제한과 폭 · 강조는 여전히 하지 않는다. 그 형식을 디자인 안에서
> 조판하는 일은 03 이다. 계약은 `RecipeV2Item.kind` · `RecipeV2Section.role` ·
> `RecipeV2Section.presentation`, 어휘는 `packages/contracts/src/recipe-vocabulary.ts`.
...
| 2026-08-31 | 디자인 문서에 한해 해시로 고정한 스크립트 하나를 CSP에서 허용 |
| 2026-08-31 | 구성 절을 없애고 그 내용을 간격 절로 합쳐 「간격과 구성」으로 |
| 2026-08-31 | 디자인 문서 CSP에 지정 호스트의 서체·사진 허용 |
| 2026-08-31 | 「공통 샘플 포트폴리오」 절을 실제 내용에 맞춰 「요소와 표시 방식」으로 |
| 2026-08-31 | Design Prompts 기반 스타일 30종을 v2 카탈로그에 흡수 · 카드 썸네일을 포스터로 |
| 2026-08-31 | 카탈로그 목록 응답에서 문서를 빼고 고른 판만 불러오도록 분리 |
| 2026-09-01 | 참고 디자인 카드 오른쪽 가장자리에 출처 회사 마크를 크게 반쯤 걸침 |
| 2026-09-01 | 모든 디자인이 같은 고품질 `DESIGN.html` 골격을 사용 · 레거시 렌더러 제거 |
| 2026-09-01 | 색 파생을 밝기 임계에서 실측 명암비로 · 스타일별 서체 지정 |
| 2026-09-01 | 스타일마다 한글 서체 지정 · 제목용과 본문용을 나눠 대체 사슬에 배치 |
| 2026-09-01 | 요소 견본이 그 디자인의 형태 · 간격 · 타입 스케일을 사용 |
| 2026-09-01 | 그림자 어휘에 offset · inset · relief · bevel 추가 |
| 2026-09-01 | `composition.layout` 축 도입 · 견본 배치가 선언을 따름 |
| 2026-09-01 | `componentKit` 어휘 도입 — 스타일마다 요소의 골격을 따로 설계 |
| 2026-09-01 | `/portfolio/new` 입구 구현 · 새 포트폴리오 링크를 v2 흐름으로 연결 |
| 2026-09-01 | 레시피는 내용과 순서만 정한다 — 표시 방식 · 요소 종류 · 폭 · 강조를 계약과 화면에서 걷어냄 |
| 2026-09-01 | 02 의 첫 화면은 빈 지면이 아니라 AI 초안이다 — 사용자가 하는 일은 조립이 아니라 고치기 |
| 2026-09-01 | 지원할 채용 공고를 02 에서 명시적으로 고른다 · 고르는 자리는 실제 공고 탐색 화면(`/jobs?pick=`) |
| 2026-09-15 | 02 가 문장의 종류 · 섹션의 역할과 보여주는 형식을 다시 정한다 — 왼쪽 내용 · 가운데 문장 · 오른쪽 형식(파란 박스 와이어프레임)의 세 영역 · 형식 목록은 디자인과 무관 |
